    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-zinc-950/55 p-4" onMouseDown={(event) => event.target === event.currentTarget && !busy && onClose()}>
      <form onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="external-plugin-title" className="w-full max-w-lg overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-[0_24px_70px_rgba(24,24,27,0.28)] dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="space-y-3 p-5">
          <label className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Prompt<textarea autoFocus rows={4} value={prompt} onChange={(event) => setPrompt(event.target.value)} className="workspace-focus mt-1.5 w-full resize-y rounded-xl border border-zinc-300 bg-zinc-100 p-3 text-sm dark:border-zinc-700 dark:bg-zinc-800" /></label>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400">{selectedCount} selected element{selectedCount === 1 ? "" : "s"} · runs in an isolated worker</p>
          {error && <p role="alert" className="rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-800 dark:bg-red-950/40 dark:text-red-200">{error}</p>}
        </div>
...
export const ExternalPluginActions = ({ canEdit, excalidrawAPI, hideTrigger = false }: PluginEditorContext) => {
  ...
  const actions = useMemo(() => plugins.flatMap((plugin) => plugin.enabled && !plugin.embedded ? (plugin.manifest.contributes?.editorActions || []).map((action) => ({ plugin, action })) : []), [plugins]);
  const actionIds = actions.map(({ plugin, action }) => `${plugin.manifest.id}:${action.id}`).join("|");
  useEffect(() => {
    const unsubscribers = actions.map(({ plugin, action }) =>
      listenForPluginAction(`${plugin.manifest.id}:${action.id}`, () => setActive({ plugin, action })),
    );
  ...
  return (
    <>
      {!hideTrigger && actions.map(({ plugin, action }) => (
