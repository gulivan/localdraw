...
export const Project = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const active = collections.find((item) => item.id === id) ?? null;
  ...
  const createSlide = async () => {
    if (!id) return;
    const drawing = await api.createDrawing(`Slide ${slides.length + 1}`, id);
    ...
    <div className="workspace-shell min-h-screen">
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center gap-3 px-4 sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            {renaming ? (
              ...
              <button type="button" onDoubleClick={() => setRenaming(true)} className="workspace-focus block max-w-full truncate rounded text-left text-sm font-bold">{project.name}</button>
            )}
            <p className="text-[11px] text-zinc-600 dark:text-zinc-400">{slides.length} slide{slides.length === 1 ? "" : "s"}</p>
          </div>
          <button type="button" onClick={() => setRenaming(true)} className="workspace-focus hidden h-9 items-center gap-1.5 rounded-xl px-3 text-xs font-semibold hover:bg-zinc-100 dark:hover:bg-zinc-800 sm:flex"><Pencil size={14} /> Rename</button>
      ...
      <main className="mx-auto max-w-6xl px-4 py-7 sm:px-6">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div><h1 className="text-2xl font-bold tracking-[-0.02em]">{project.name}</h1><div className="mt-3 flex gap-2">{PROJECT_COLORS.map((color) => <button key={color} type="button" onClick={() => void updateColor(color)} aria-label={`Use project color ${color}`} aria-pressed={project.color === color} className={`workspace-focus h-6 w-6 rounded-full border-2 border-white dark:border-zinc-950 ${project.color === color ? "ring-2 ring-zinc-900 ring-offset-1 dark:ring-zinc-100 dark:ring-offset-zinc-950" : ""}`} style={{ backgroundColor: color }} />)}</div></div>
          <button type="button" onClick={() => void removeProject()} className="workspace-focus flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40"><Trash2 size={14} /> Delete project</button>
        </div>
        {slides.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {slides.map((slide, index) => <ProjectSlideCard key={slide.id} slide={slide} index={index} projects={projects} canOrganize onOpen={() => navigate(`/editor/${slide.id}`)} onRename={() => void renameSlide(slide)} onDelete={() => void moveSlideToTrash(slide.id)} onMove={(collectionId) => void place(slide.id, collectionId, projects.find((item) => item.id === collectionId)?.drawingCount ?? 0)} onReorder={(targetIndex) => void place(slide.id, id || null, targetIndex)} onDropAt={(draggedId, targetIndex) => void place(draggedId, id || null, targetIndex)} />)}
          </div>
        )}
      </main>
      <UploadStatus />
