...
- Manual dispatch of `desktop-release.yml` is a recovery path for an existing tag/release, not the normal release process. After every native asset succeeds and the GitHub Release is published, `desktop-release.yml` dispatches `.github/workflows/npm-publish.yml` as a separate trusted-publishing workflow and waits for it. Keep npm publishing in that standalone workflow; nested reusable publishing is rejected by npm trusted publishing. The npm workflow safely no-ops when that version already exists on npm.
